...
static void BM_RedisCpp_Incr(benchmark::State& state) {
    ...
    try {
        ...
        rediscpp::execute(*stream, "SET", key, "0");

        for (auto _ : state) {
            // Increment the counter
            auto response = rediscpp::execute(*stream, "INCR", key);

...
static void BM_RedisCpp_Hget(benchmark::State& state) {
    ...
    try {
        ...
        rediscpp::execute(*stream, "HSET", key, field, value);

        for (auto _ : state) {
            // Get a field value from a hash
            auto response = rediscpp::execute(*stream, "HGET", key, field);
